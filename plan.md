...
## Phase 2: トラッキング (Tracking) (完了)
- [x] **等速運動モデル**: 前フレームの速度から現在のカメラ姿勢を予測する。
- [x] **参照キーフレーム追跡**: 現在のフレームと直近のキーフレーム間で特徴点マッチングを行う。
- [x] **局所地図追跡**:
    - 共視グラフ(Covisibility Graph)上の近傍キーフレームから、ランドマークを現在フレームに投影。
    - SolvePnPRansacを用いて姿勢を最適化。
- [x] **キーフレーム決定**: 新しいキーフレームを挿入するかどうかの判定 (スタブ実装済)。

## Phase 3: 初期化 (Initialization) (完了)
- [x] **単眼初期化**:
...
## Phase 4: 局所マッピング (Local Mapping / Backend) (進行中)
- [x] **キーフレーム挿入**: 新しいキーフレームをMapに追加し、接続関係を更新。
- [ ] **マップポイント選別**: 重複や品質の悪いマップポイントを削除 (Culling)。
- [x] **新規マップポイント作成**: 接続されたキーフレーム間で未マッチの特徴点から新規点を三角測量。
- [ ] **局所バンドル調整 (Local BA)**:
